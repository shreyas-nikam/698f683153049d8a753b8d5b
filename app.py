...
def _initialize_session_state():
    ...
    if "end_date" not in st.session_state:
        st.session_state.end_date = pd.to_datetime(DEFAULT_END_DATE)
    ...
    if "sector_map_cleaned" not in st.session_state:
        st.session_state.sector_map_cleaned = {}
    if "n_components_pca" not in st.session_state:
        st.session_state.n_components_pca = 5
    if "R_pca" not in st.session_state:
        st.session_state.R_pca = np.array([])
    if "pca_model" not in st.session_state:
        st.session_state.pca_model = None
    if "k_range_values" not in st.session_state:
        st.session_state.k_range_values = range(2, 11)
    ...
    if "K_optimal" not in st.session_state:
        st.session_state.K_optimal = 5
    if "kmeans_cluster_labels" not in st.session_state:
        st.session_state.kmeans_cluster_labels = None
    if "kmeans_model" not in st.session_state:
        st.session_state.kmeans_model = None
    if "hc_cluster_labels" not in st.session_state:
        st.session_state.hc_cluster_labels = None
    if "cluster_profile_summary" not in st.session_state:
        st.session_state.cluster_profile_summary = pd.DataFrame()
    if "asset_assignment_table" not in st.session_state:
        st.session_state.asset_assignment_table = pd.DataFrame()
    if "intra_corrs_mean" not in st.session_state:
        st.session_state.intra_corrs_mean = None
    if "inter_corrs_mean" not in st.session_state:
        st.session_state.inter_corrs_mean = None
    if "cluster_portfolio_vol" not in st.session_state:
        st.session_state.cluster_portfolio_vol = None
    if "random_portfolio_vol" not in st.session_state:
...
if st.session_state.page == "Introduction":
    ...
    **Connection to Hierarchical Risk Parity (HRP)**: The Lopez de Prado (2020) HRP method—now widely adopted by quantitative allocators—uses hierarchical clustering of the correlation matrix as its core step. This case study introduces the foundational technique underlying HRP, which participants can extend in D5-T1-C2 (AI-Driven Portfolio Optimization).
...
elif st.session_state.page == "1. Data Acquisition & Preparation":
    ...
    Based on visual inspection of these plots, David observes that a `K_optimal` around 5 or 6 seems appropriate, as the Elbow plot shows a distinct bend and the Silhouette score reaches a reasonable peak in that range. For the purpose of this exercise, David decides to proceed with `K_optimal = {st.session_state.K_optimal}`, balancing interpretability with statistical goodness-of-fit. This data-driven selection of `K` is a significant improvement over arbitrary groupings, ensuring his clusters are statistically meaningful.
    """)

elif st.session_state.page == "4. Hierarchical Clustering":
    ...
    This nested structure provides a richer story than the "flat" partitions of K-Means. It helps David understand the tiered relationships among assets, which is critical for implementing sophisticated diversification strategies like Hierarchical Risk Parity. The `fcluster` function allowed us to "cut" this dendrogram to retrieve `{st.session_state.K_optimal}` clusters, providing a direct comparison and potential validation for our K-Means results.
    """)
